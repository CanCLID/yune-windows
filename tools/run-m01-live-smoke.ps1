...
if ($PreflightOnly) {
    ...
    if ($PreflightPath -eq "") {
        $PreflightPath = Join-Path $env:TEMP "yune-windows\m01-live-preflight.json"
    }
...
Require-ApprovedMachineStateChange `
    ...
    -Action "run the M01 install/register/smoke/diagnostics/uninstall sequence"
...
$TsfEvidence = Join-Path $EvidenceRoot "m01\tsf-smoke"
$DiagnosticsEvidence = Join-Path $EvidenceRoot "m01\settings"
...
function Write-LiveSmokeResult {
    ...
    $Lines += @(
        ...
        "",
        "Notepad smoke: see ``docs/evidence/m01/tsf-smoke/notepad-smoke-result.md`` when present.",
        "",
        "Chromium smoke: see ``docs/evidence/m01/tsf-smoke/chromium-smoke-result.md`` when present.",
        "",
        "Diagnostics bundle:",
        "",
        "````text",
...
$LivePreflightCommand = "tools\run-m01-live-smoke.ps1 -PreflightOnly -PreflightPath $(Format-CommandValue $LivePreflightPath) -YuneRoot $(Format-CommandValue $YuneRoot) -InstallDir $(Format-CommandValue $InstallDir) -RefreshCurrentResidue -ApprovalNote $(Format-CommandValue $ApprovalNote)"
...
try {
    ...
    try {
        ...
        if ($LiveSmokeSucceeded) {
            ...
            try {
                $AuditCommand = "tools\audit-m01-closeout.ps1 -RequireComplete"
                Record-Command $AuditCommand
                & (Join-Path $RepoRoot "tools\audit-m01-closeout.ps1") -RequireComplete | Out-Null
...
Write-Host "M01 live smoke sequence finished. Review $ResultPath and $CleanupPath."

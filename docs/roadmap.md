...
| **M11 — UI aesthetic modernization + Cantonese localization (planned)** | Themes the settings panel to Win11-native (v6 manifest + JhengHei font + DPI, then DWM Mica/rounded/dark), moves the toolbar to a DirectComposition frosted-glass renderer, and localizes **all** user-facing text English→written Cantonese mirroring `yune-web` (`uiText.yue`). Native, no WebView2. | M–L | **Risks locked in the plan:** supported backdrops blur the desktop wallpaper, not the live app (live blur needs undocumented `SetWindowCompositionAttribute` — spike decides); settings combos couple display text to the server value (needs a label/value split); ULW→DComp drops per-pixel-alpha click-through (needs `WS_EX_NOREDIRECTIONBITMAP` + explicit hit-testing). Tier-3 fully-custom panel deferred but not foreclosed. Plan: `docs/plans/active/m11-plan-ui-modernization-cantonese.md`. |

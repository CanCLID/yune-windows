...
| --- | --- | --- |
| Product shell | Renamed public baseline with TSF DLL, shared server, native candidate window, diagnostics tooling, installer scripts, non-elevated contract tests, M02 product-owned server startup, M03 development inner-loop tooling, M04 candidate typing-quality implementation, and M05 IME controls implementation. Latest approved live closeout reached install/register, profile activation, Notepad, Chromium, diagnostics export, uninstall, and post-reboot no-residue cleanup. M04 runtime evidence covers clean candidate comments, 30-candidate server supply, full-width punctuation via `get_commit`, and installed-server reload/readiness. M05 adds server-owned persistent schema/options state in `state\ime-state.json`, `op=` IPC verbs, state reconciliation on every server response, lone-Shift and preserved-key toggles, a focus-scoped native mini language bar, and `YuneWindowsSettings.exe`. M05 review crash blockers are fixed in non-elevated scratch-server coverage: `ascii_mode=true` non-empty input, persisted ascii restart, invalid op/schema/option requests, and settings/schema-cycle fallback no longer kill the shared server. M06 is complete, with live Tier-1 host typing proof on 2026-07-02. M07 persistent composition is confirmed live for partial selection and inline preedit. Compact milestone summaries live under `docs/evidence/m01/` through `docs/evidence/m11c/`; bulky raw artifacts are regenerated only when a fresh gate needs them. M08 delivered the native draggable toolbar foundation, M09 added the settings segment/native settings panel/skin picker, and M11 covers Cantonese UI, Win11 settings polish, and the DirectComposition toolbar. The approved M11 installed clone/drag sub-gate passed with one foreground-owned HWND and no fresh visual copies. M11D boot-ID/revision CAS, process-local token/parity routing, deadline-bound operation I/O, and deterministic visibility logic now pass non-elevated gates. The M11 settings DPI/resize/scroll repair also passes its non-elevated smoke. | **Stop-the-line M11 closeout:** build and deploy the combined hash-pinned tree, then complete Notepad/Chromium/Explorer/Electron activation/visibility plus installed settings usability while preserving the clone-free result. M10 remains blocked and still owns skin breadth plus candidate rendering. Later: dogfood packaging, cold-start/broker. |
...
M02 through M09 are complete. The installed **M11 clone/drag sub-gate passed**.
**M11D remains the active stop-the-line track** because activation, lone-Shift
state transition, and toolbar visibility were not deterministic in Chromium,
Explorer, or Claude. Its runtime repair and the M11 settings DPI/resize/scroll
repair now pass non-elevated gates. **M10 is blocked** until the combined
hash-pinned tree completes installed settings usability and the four-host gate;
M10 continues to own skin breadth plus candidate-window rendering.

| Candidate | Delivers | Rough size | Key dependency / risk |
| --- | --- | --- | --- |
| **M11D — IME activation, toggle, and toolbar-visibility reliability (implementation fixed; installed proof pending)** | Separates Windows profile activation, focused TSF service identity, token/disposition-based sink/hook routing, boot-ID/revision CAS, deadline-bound worker I/O, and toolbar show eligibility. Non-elevated CAS/token/parity/visibility gates pass. | M | Chromium needed nine attempts, Explorer needed five and then eleven, and Claude showed no toolbar after twelve attempts. The approval-gated 50-toggle/four-host installed proof remains required before M10. Plan: `docs/plans/active/m11d-ime-activation-toolbar-visibility.md`. |
| **M08 — Modern floating toolbar (superseded presentation path; installed clone/drag proof recorded under M11)** | Replaced the flat GDI language bar with a draggable, position-remembering, Direct2D/DirectWrite toolbar and established the native interaction/position foundation later migrated by M11. | M | M08's presentation path was superseded by M11; current installed single-toolbar/drag evidence is recorded under M11/M11C. Evidence: `docs/evidence/m08/summary.md`; historical plan: `docs/plans/history/m08-plan-modern-toolbar.md`. |
| **M09 — Settings panel + skin picker (implementation complete; M11 installed usability closeout pending)** | Added a toolbar settings segment and a **native Win32 settings panel** covering supported session toggles, schema switch, skin picker, and shared-renderer preview; engine prefs / userdb import-export / schema import stay scaffolded present-but-disabled and future-ready. | M | The live surface exposed the DPI cutoff; M11 owns the implemented DPI/resize/scroll repair and its installed usability closeout. UI remains native Win32 (D-16). Evidence: `docs/evidence/m09/summary.md`; historical plan: `docs/plans/history/m09-plan-skins-and-picker.md`. |
| **M10 — Skin breadth + candidate-window skinning (blocked)** | Manifest-only V1 skins; shared bounded `SkinDefinition`/`SkinCatalog`; strict user imports and revisioned IPC; built-in equal-sized `midnight`; opaque/static-tint DComp candidate restyle with normally redirected GDI first-show fallback. | M | Does not begin until M11D and installed settings usability pass the combined hash-pinned host gate. M10 owns candidate behavior/performance evidence (30 cold shows, 500 warm updates); M11 supplies only the reusable composition foundation. Plan: `docs/plans/active/m10-plan-skin-breadth-candidate-window.md`. |
| **M11 — UI aesthetic modernization + Cantonese localization (clone/drag proven; reliability/settings implementation fixed; installed closeout pending)** | Themes/localizes the native settings panel and toolbar, ports the toolbar to DirectComposition + Direct2D, stabilizes real-window topology and movement-only drag, and adds DPI-aware resizable/scrollable settings layout. | M–L | Installed Notepad/Chromium clone proof retained one valid foreground-owned HWND. The combined hash-pinned activation/visibility, Explorer/Electron, restart-persistence, and settings-usability gate remains open. Plan/evidence: `docs/plans/active/m11-plan-ui-modernization-cantonese.md`, `docs/plans/active/m11c-dcomp-glass-toolbar-handoff.md`, `docs/evidence/m11/summary.md`, `docs/evidence/m11c/summary.md`. |

# M04 Candidate Window And Typing Quality Implementation Plan
...
`docs/evidence/m04/summary.md` and `docs/evidence/m04/summary.json`.

**Architecture:** iterate through the M03 dev loop. Comment hygiene is
...
- **Loop reality (from M03):** once the profile is active, the TSF DLL loads
...
- Do not weaken any M03 dev-tooling safety contract or any elevated
  approval gate.
...
- [ ] Live-verify via `dev-reload-tsf` in a holder-free session that the panel
  ...
  by non-dev TSF DLL holders; see `docs\evidence\m04\`.
...
- [ ] Live-verify paging reaches later candidates in an app. Blocked in this
  ...
  candidate-window smoke coverage are captured under `docs\evidence\m04\`.
- [x] Commit directly to `main`.
...
- Live verification captured under `docs\evidence\m04\`; DLL-side slices

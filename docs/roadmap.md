...
| --- | --- | --- |
| Product shell | M01-M09 are complete. Implementation last changed at `4199a09`; pre-rebaseline source/evidence baseline `337b9bd` contains the native Cantonese UI, DirectComposition toolbar, ownership/clone/drag repair, deterministic activation/state machinery, and settings DPI/resize/scroll repair, with focused non-elevated checks passing. The approved older-build clone/drag run retained one foreground-owned HWND without fresh visual copies, but the current implementation baseline has not been installed. D-21 rebaselines the active work as M10 presentation, M11 activation/state reliability, M12 skin platform, and M13 candidate presentation while preserving legacy `m11*` evidence labels. | **Paired M10 + M11 closeout:** complete the expanded M11 pre-deployment matrix, then use one separately approved hash-pinned deployment and record separate toolbar/settings and four-host activation/state verdicts. Both must pass before M12. M13 follows M12; its renderer migration waits for a reproducible GDI candidate baseline. Later: dogfood packaging, cold-start/broker. |
...
inversion. M10 and M11 form one paired hash-pinned installed closeout with
separate verdicts; a host M10 cannot exercise because M11 cannot show remains
open, not passed. M12 begins only after both pass;
...
| --- | --- | --- | --- |
| **M10 — Native UI Presentation Closeout (implementation complete; installed acceptance pending)** | Native Cantonese settings/toolbar UI, DirectComposition toolbar, owner-valid singular presentation, movement-only drag/backdrop fallback, and DPI-aware resizable/scrollable settings usability. | M | The older `1f419837` clone result is regression evidence, not current-build acceptance. Record separate current-hash toolbar-presentation and settings-usability verdicts. Plan: `docs/plans/active/m10-native-ui-presentation-closeout.md`. |
| **M11 — Activation and State Reliability (implementation complete; focused checks pass; expanded pre-deployment and installed acceptance pending)** | Focus-generation authority, sink/hook token and rapid-input parity, boot-ID/revision CAS, bounded IPC/reconciliation, privacy-safe tracing, and deterministic toolbar eligibility. | M | Current core/concurrency smokes are not end-to-end multiprocess TSF proof. Complete the expanded non-elevated matrix, then require no sacrificial toggle, 50 paced exact transitions, parity-correct bursts, and four-host focus/hide proof. Plan: `docs/plans/active/m11-activation-state-reliability.md`. |

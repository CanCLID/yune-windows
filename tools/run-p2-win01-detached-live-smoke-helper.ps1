...
try {
    ...
    Require-ApprovedMachineStateChange `
        ...
        -Action "run the detached P2-WIN01 live smoke helper"
    ...
    Write-HelperResult -StatusValue "running" -StageValue $Stage -ExitCodeValue $null
    & (Join-Path $RepoRoot "tools\start-p2-win01-elevated-live-smoke.ps1") `
        -YuneRoot $YuneRoot `
        -InstallDir $InstallDir `
    ...
    $Stage = "final-audit"
    Write-HelperResult -StatusValue "running" -StageValue $Stage -ExitCodeValue $null
    & (Join-Path $RepoRoot "tools\audit-p2-win01-closeout.ps1") -RequireComplete | Out-Host

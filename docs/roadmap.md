...
| --- | --- | --- |
| Product shell | M01-M09 are complete. Implementation last changed at `eb262fa`; the native Cantonese UI, DirectComposition toolbar, ownership/clone/drag repair, shared toolbar eligibility, typed persistence failures, settings four-DPI resize/scroll repair, disposition tracing, and durable frozen-candidate closeout tooling pass focused non-elevated checks. The approved older-build clone/drag run retained one foreground-owned HWND without fresh visual copies, but the current implementation baseline has not been installed. D-21 owns the M10-M13 scope map; D-22 permits M10 deployment readiness to proceed independently of M11 acceptance readiness. | **M10 closeout now:** publish and deploy one hash-pinned candidate, then record separate toolbar-presentation and settings-usability verdicts. **M11 next:** complete its remaining direct-TSF pre-deployment matrix and installed activation/state verdict; reuse the M10 candidate only if all product/package hashes remain exact. Both must pass before M12. M13 follows M12; its renderer migration waits for a reproducible GDI candidate baseline. Later: dogfood packaging, cold-start/broker. |
...
| --- | --- | --- | --- |
| **M10 — Native UI Presentation Closeout (implementation and focused preflight complete; installed acceptance pending)** | Native Cantonese settings/toolbar UI, DirectComposition toolbar, owner-valid singular presentation, movement-only drag/backdrop fallback, and DPI-aware resizable/scrollable settings usability. | M | The older `1f419837` clone result is regression evidence, not current-build acceptance. Deploy and record separate current-hash toolbar-presentation and settings-usability verdicts. Plan: `docs/plans/active/m10-native-ui-presentation-closeout.md`. |

# P2-WIN05 IME Toggles, Language Bar, and Settings Implementation Plan
...
settings exe. Iterate via the P2-WIN03 dev loop (server = instant
...
### Task 0: Prerequisites (before DLL-side WIN05 work)
- [ ] Record the P2-WIN04 DLL-side live proof under `docs\evidence\p2-win04\`
  (the holder-free `dev-reload-tsf` swap plus confirmed caret positioning,
  no-orphan panel, `-`/`=` paging, clean comments, punctuation) and update the
  roadmap so the "holder-free P2-WIN04 live verification" gate is closed before
...
- No Yune ABI widening; `disable_learning` stays forced; inline candidate window
  stays native. Live DLL-side verification captured under `docs/evidence/p2-win05/`.

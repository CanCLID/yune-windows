...
foreach ($GateId in @("tsf-notepad-smoke", "chromium-text-field-smoke", "candidate-display-live")) {
    ...
    if ($Gate.status -ne "invalid") {
        throw "audit should reject identical candidate/commit screenshots for $GateId, got $($Gate.status)"
    }
}

Write-Host "Closeout audit rejects identical candidate/commit screenshots."

...
| --- | --- | --- |
| Product shell | M01-M09 are complete. Implementation last changed at `589bc3d`; the native Cantonese UI, DirectComposition toolbar, ownership/clone/drag repair, refresh-before-re-show position persistence, shared toolbar eligibility, typed persistence failures, settings four-DPI resize/scroll repair, disposition tracing, and durable frozen-candidate closeout tooling pass focused non-elevated checks. Corrective candidate `589bc3d` is installed with exact artifact hashes and frozen at `deployed_restart_required`; post-restart verification and both M10 installed verdicts remain pending. Candidate `7622305` remains regression/repro evidence only after failing cross-host position persistence. D-21 owns the M10-M13 scope map; D-22 keeps M10 and M11 acceptance separate. | **M10 closeout now:** reboot, run the bound post-restart verifier, then record separate toolbar-presentation and settings-usability verdicts on the frozen candidate. **M11 next:** complete its remaining direct-TSF pre-deployment matrix and installed activation/state verdict; reuse the M10 candidate only if all product/package hashes remain exact. Both must pass before M12. M13 follows M12; its renderer migration waits for a reproducible GDI candidate baseline. Later: dogfood packaging, cold-start/broker. |
...
import. The candidate window remains visually unchanged until M13. See
`docs/evidence/m09/summary.md`.

Historical plan:
`docs/plans/history/m09-plan-skins-and-picker.md`.

## Active and Candidate Milestones

M02 through M09 are complete. D-21 removes the historical M11-before-M10
inversion; D-22 permits M10 to take its installed presentation/settings gate
before M11's remaining direct-TSF preflight. The verdicts remain separate, and
a host M10 cannot exercise because the toolbar cannot show remains open, not
passed. M12 begins only after both pass;
M13 begins after M12 with its GDI candidate baseline slice. See
`docs/reference/m10-m13-rebaseline.md`.

| Milestone | Delivers | Rough size | Key dependency / risk |
| --- | --- | --- | --- |
| **M10 — Native UI Presentation Closeout (corrective candidate deployed; restart and installed acceptance pending)** | Native Cantonese settings/toolbar UI, DirectComposition toolbar, owner-valid singular presentation, movement-only drag/backdrop fallback, and DPI-aware resizable/scrollable settings usability. | M | Corrective candidate `589bc3d` is frozen at `deployed_restart_required`; its durable manifest and measured preflight are recorded separately. Reboot, verify the exact installed candidate, and record separate toolbar-presentation and settings-usability verdicts. Plan: `docs/plans/active/m10-native-ui-presentation-closeout.md`. |
| **M11 — Activation and State Reliability (implementation complete; focused checks and expanded server fault/reconciliation smoke pass; direct-TSF and installed acceptance pending)** | Focus-generation authority, sink/hook token and rapid-input parity, boot-ID/revision CAS, bounded IPC/reconciliation, privacy-safe tracing, and deterministic toolbar eligibility. | M | Current core/concurrency smokes are not end-to-end multiprocess TSF proof. Complete the remaining direct-TSF matrix, then require no sacrificial toggle, 50 paced exact transitions, parity-correct bursts, and four-host focus/hide proof. Plan: `docs/plans/active/m11-activation-state-reliability.md`. |
| **M12 — Skin Platform (planned; blocked)** | Strict shared `SkinDefinition`/`SkinCatalog`, equal-sized `midnight`, transactional manifest import, bounded validation, revisioned IPC/cache, lifetime rules, and catalog-backed settings. | M | Begins only after both M10 and M11 installed gates pass. V1 is manifest-only and changes no Yune ABI. Plan: `docs/plans/active/m12-skin-platform.md`. |
| **M13 — Candidate Presentation (planned; blocked)** | Reproducible GDI baseline, generalized composition lifecycle, complete skin-driven opaque/static DComp candidate, normally redirected GDI fallback, `頁 n/N`, behavior preservation, and latency gates. | M | Begins after M12 with 30-cold/500-warm Slice 0 baseline capture; renderer migration waits for reproducibility. Candidate evidence remains independent from toolbar evidence. Plan: `docs/plans/active/m13-candidate-presentation.md`. |

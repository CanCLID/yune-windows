...
if ($OutputDir -eq "") {
    $OutputDir = Join-Path $env:TEMP "yune-windows\p2-win01-audit-expected-commit-quality-test"
}
...
Committed text: arbitrary-non-raw-text
Matches expected Yune commit: False
"@
}
Write-EvidenceBytes "p2-win01-tsf-smoke\candidate-display-notepad.png" $ValidPng
Write-EvidenceBytes "p2-win01-tsf-smoke\notepad-commit.png" $ValidPng
Write-EvidenceBytes "p2-win01-tsf-smoke\candidate-display-chromium.png" $ValidPng
Write-EvidenceBytes "p2-win01-tsf-smoke\chromium-commit.png" $ValidPng
...
foreach ($GateId in @("tsf-notepad-smoke", "chromium-text-field-smoke")) {
    ...
    if ($Gate.status -ne "invalid") {
        throw "audit should reject $GateId evidence without expected Yune commit proof, got $($Gate.status)"
    }
}

Write-Host "Closeout audit rejects text-field smoke evidence without expected Yune commit proof."

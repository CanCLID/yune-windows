...
| **M11 — UI aesthetic modernization + Cantonese localization (non-elevated implementation)** | Themes the settings panel to Win11-native (v6 manifest + JhengHei font + DPI, then DWM Mica/rounded/dark), localizes the settings panel and toolbar glyphs to written Cantonese, splits settings combo display labels from server values, and ports the language-bar toolbar to native DirectComposition + Direct2D over DWM Desktop Acrylic. Native, no WebView2. | M–L | Live TSF-host visual proof of clone-free repeated drag, frosted glass over a real host, and no focus steal remains human-gated. Non-elevated source/build/contracts guard `WS_EX_NOREDIRECTIONBITMAP`, fixed 96-DPI D2D target, `D2D1_BITMAP_OPTIONS_CANNOT_DRAW`, and DWM acrylic attributes. Plan/evidence: `docs/plans/active/m11-plan-ui-modernization-cantonese.md`, `docs/plans/active/m11c-dcomp-glass-toolbar-handoff.md`, `docs/evidence/m11/summary.md`, `docs/evidence/m11c/summary.md`. |

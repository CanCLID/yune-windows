...
if ($OutputDir -eq "") {
    $OutputDir = Join-Path $env:TEMP "yune-windows\p2-win01-audit-diagnostics-quality-test"
}
...
function Write-DiagnosticsBundleWithoutStructuralLogs([string]$RelativePath) {
    ...
    "synthetic diagnostics bundle without structural TSF logs" |
...
"@
Write-DiagnosticsBundleWithoutStructuralLogs "p2-win01-settings\registered-session-diagnostics\missing-structural-log.zip"
...
if ($DiagnosticsGate.status -ne "invalid") {
    throw "audit should reject diagnostics bundle evidence without structural TSF logs, got $($DiagnosticsGate.status)"
}

Write-Host "Closeout audit rejects registered-session diagnostics bundles without structural TSF logs."

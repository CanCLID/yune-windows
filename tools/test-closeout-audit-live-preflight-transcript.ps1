...
if ($OutputDir -eq "") {
    $OutputDir = Join-Path $env:TEMP "yune-windows\m01-audit-live-preflight-transcript-test"
}
...
if (-not ($Commands -match 'run-m01-live-smoke\.ps1 -PreflightOnly')) {
    throw "complete synthetic fixture must include live-preflight transcript entries"
}
$Commands |
    Where-Object { $_ -notmatch 'run-m01-live-smoke\.ps1 -PreflightOnly' } |
...
$WrongPathCommandsPath = Join-Path $WrongPathEvidenceRoot "m01\installer\commands.txt"
$ExpectedPreflightPath = [regex]::Escape("$WrongPathEvidenceRoot\m01\installer\live-preflight.json")
$WrongPreflightPath = "$WrongPathEvidenceRoot\m01\installer\unreviewed-live-preflight.json"

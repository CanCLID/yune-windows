...
if ($OutputPath -eq "") {
    $OutputPath = Join-Path $RepoRoot "docs\evidence\m01\installer\compatibility-environment.json"
}
...
$Environment = [ordered]@{
    ...
    compatibility_target_id = "M01-WIN11-X64"
    ...
    live_status = "covered-by-m02-summary"
    live_closeout_evidence = "docs/evidence/m02/summary.json"
    m01_closes = $true
}

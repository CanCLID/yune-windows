...
if ($OutputDir -eq "") {
    $OutputDir = Join-Path $env:TEMP "yune-windows\p2-win01-audit-typed-input-test"
}
...
foreach ($ResultPath in @($NotepadResult, $ChromiumResult)) {
    ...
    $Text = $Text -replace "Input method: Win32 virtual-key typed test input\.\r?\n", ""
    $Text | Out-File -LiteralPath $ResultPath -Encoding utf8
}

$JsonPath = Join-Path $OutputDir "audit-without-typed-input.json"
$MarkdownPath = Join-Path $OutputDir "audit-without-typed-input.md"
...
if ($NotepadGate.status -eq "complete" -or $ChromiumGate.status -eq "complete") {
    throw "closeout audit should reject text-smoke evidence that does not record typed-input method; got notepad=$($NotepadGate.status), chromium=$($ChromiumGate.status)"
}

Write-Host "Closeout audit rejects text-smoke evidence without typed-input method proof."

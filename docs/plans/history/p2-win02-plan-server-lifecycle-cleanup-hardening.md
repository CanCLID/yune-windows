# P2-WIN02 Server Lifecycle And Cleanup Hardening Implementation Plan
...
`codex/p2-win02-server-lifecycle-cleanup`.
...
- P2-WIN01 proved the core development dogfood path under the Yune Windows
...
- Manual dogfood also exposed profile activation loss:
  ...
  `active=false` between setup and the target text field. P2-WIN02 must not
...
- The current smoke scripts call `tools\start-yune-windows-server.ps1` before
  typing. That is acceptable for pre-P2-WIN02 harnesses but not for the product
  dogfood contract.
...
- Do not add a Windows service, scheduled task, Run-key autostart, or other
  persistent server launcher in the first P2-WIN02 implementation slice.
...
- Modify `tools\run-p2-win01-live-smoke.ps1`
...
- Create `docs\evidence\p2-win02-server-lifecycle\`
  - New evidence directory for P2-WIN02 preflight, smoke, live, and cleanup
    artifacts.
- Modify `README.md`
  - Replace the manual server-start dogfood workaround after P2-WIN02 passes.
...
be blocked by Defender ASR, SmartScreen, or endpoint policy; P2-WIN02 should
...
## Task 1: Baseline P2-WIN02 Plan And Contracts

**Files:**
- Create: `docs\evidence\p2-win02-server-lifecycle\baseline.md`
- Create: `tools\test-tsf-server-autostart-contract.ps1`
- Create: `tools\test-tsf-server-failure-reason-contract.ps1`
- Create: `tools\test-yune-server-single-instance-contract.ps1`
- Create: `tools\test-product-owned-server-smoke-contract.ps1`
...
In `tools\run-p2-win01-live-smoke.ps1`, pass:
...
```powershell
git add tools\uninstall-yune-windows-ime.ps1 tools\run-p2-win01-live-smoke.ps1 tools\live-smoke-support.ps1
...
- Modify: `docs\plans\active\p2-win02-plan-server-lifecycle-cleanup-hardening.md`
...
Move P2-WIN02 from recommended to active/in-progress while implementation is
...
```powershell
git add README.md docs\roadmap.md docs\requirements.md docs\decisions.md docs\plans\active\p2-win02-plan-server-lifecycle-cleanup-hardening.md
...
**Files:**
- Read: `docs\evidence\p2-win02-server-lifecycle\`
- Modify: evidence files only when commands produce new P2-WIN02 evidence
...
**Files:**
- Modify: `docs\evidence\p2-win02-server-lifecycle\`
- Modify: `docs\evidence\p2-win01-installer\` only if the existing live-smoke
  scripts still write there
- Read: `docs\evidence\p2-win01-closeout\audit.json`
...
```powershell
powershell -STA -NoProfile -ExecutionPolicy Bypass -File tools\run-p2-win01-live-smoke.ps1 -ApprovedMachineStateChange -ApprovalNote "User approved P2-WIN02 live install/register/smoke/uninstall/cleanup in this session." -YuneRoot C:\Users\laubonghaudoi\Documents\GitHub\yune
```
...
- if Defender ASR, SmartScreen, or other endpoint policy blocks the unsigned
  server launch, the transcript and structural logs record that as a P2-WIN02
...
Run the closeout audit command used by P2-WIN01. If a new P2-WIN02 audit script
is created during implementation, run that instead and keep P2-WIN01 evidence
unchanged.
...
P2-WIN02 is complete only when all of these are true:

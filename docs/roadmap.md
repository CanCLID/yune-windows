...
| **M11 — UI aesthetic modernization + Cantonese localization (planned)** | Themes the settings panel to Win11-native (v6 manifest + JhengHei font + DPI, then DWM Mica/rounded/dark), moves the toolbar to a WinRT `Windows.UI.Composition` frosted-glass renderer, and localizes **all** user-facing text English→written Cantonese mirroring `yune-web` (`uiText.yue`). Native, no WebView2. | M–L | **Risks locked in the plan:** live blur-behind uses the **documented host backdrop brush** (`DWMWA_USE_HOSTBACKDROPBRUSH` + `CreateHostBackdropBrush`, Win11 22000+, `WS_EX_NOREDIRECTIONBITMAP`) first, with undocumented accent acrylic / DWM wallpaper acrylic / static tint as fallbacks (spike locks the primary); settings combos couple display text to the server value (needs a label/value split); ULW→composition drops per-pixel-alpha click-through (needs explicit hit-testing). Tier-3 fully-custom panel deferred but not foreclosed. Plan: `docs/plans/active/m11-plan-ui-modernization-cantonese.md`. |

...
8. **IME toggles, language bar, and settings implementation** - server-owned
   ...
   native `YuneWindowsSettings.exe`. Non-elevated runtime and contract evidence
   is summarized in `docs/evidence/m05/summary.md`; holder-free live Notepad and
...
| --- | --- | --- | --- |
| **M04/M05 holder-free live verification** | Prove the implemented caret placement, no-orphan behavior, PageUp/PageDown paging, full-sentence punctuation, lone-Shift mode toggle, preserved-key toggles, focus-scoped language bar, and settings-driven state changes in Notepad and Chromium after the TSF DLL can be reloaded without non-dev holders. | S | Requires closing holder apps, sign-out, or reboot to produce a holder-free desktop session; do not force-close non-dev apps from tooling. Evidence roots: `docs/evidence/m04/` and `docs/evidence/m05/`. |

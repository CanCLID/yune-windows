...
if ($OutputDir -eq "") {
    $OutputDir = Join-Path $env:TEMP "yune-windows\p2-win01-audit-quality-test"
}
...
Write-EvidenceFile "p2-win01-tsf-smoke\notepad-smoke-result.md" "Pass: True"
Write-EvidenceFile "p2-win01-tsf-smoke\chromium-smoke-result.md" "Pass: True"
Write-EvidenceFile "p2-win01-installer\result.md" "Fresh install: attempted"
Write-EvidenceFile "p2-win01-installer\cleanup-result.md" "Pass: True"
...
foreach ($GateId in @(
    ...
    if ($Gate.status -ne "invalid") {
        throw "audit should mark placeholder evidence for $GateId invalid, got $($Gate.status)"
    }
}

$CandidateGate = $Audit.gates | Where-Object { $_.id -eq "candidate-display-live" } | Select-Object -First 1
if (-not $CandidateGate) {
    throw "audit did not emit candidate-display-live gate"
}
if ($CandidateGate.status -ne "invalid") {
    throw "audit should mark missing candidate-display screenshots invalid when live result placeholders exist, got $($CandidateGate.status)"
}

Write-Host "Closeout audit rejects placeholder live evidence."

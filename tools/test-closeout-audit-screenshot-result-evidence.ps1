...
foreach ($ResultPath in @(
    ...
    Remove-ResultLine `
        ...
        -LinePattern "Candidate-display screenshot captured: True"
    Remove-ResultLine `
        -RelativePath $ResultPath `
        -LinePattern "Commit screenshot captured: True"
}
...
foreach ($GateId in @(
    ...
    if ($Gate.status -ne "invalid") {
        throw "audit should reject text-smoke evidence without screenshot-capture result proof for $GateId, got $($Gate.status)"
    }
}

Write-Host "Closeout audit rejects text-smoke evidence without screenshot-capture result proof."

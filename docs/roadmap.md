...
**M11D is the active stop-the-line track** because activation, lone-Shift state
transition, and toolbar visibility were not deterministic in Chromium, Explorer,
or Claude. **M10 is blocked** until M11D completes the four-host gate and
continues to own skin breadth plus candidate-window rendering.

| Candidate | Delivers | Rough size | Key dependency / risk |
| --- | --- | --- | --- |
| **M11D — IME activation, toggle, and toolbar-visibility reliability (active)** | Separates Windows profile activation, focused TSF service identity, exactly-once lone-Shift routing, acknowledged server state, and toolbar show eligibility. Adds privacy-safe reason tracing and preserves all M11C clone invariants. | M | Chromium needed nine attempts, Explorer needed five and then eleven, and Claude showed no toolbar after twelve attempts. Must pass 50 exact toggles and the complete four-host installed gate before M10. Plan: `docs/plans/active/m11d-ime-activation-toolbar-visibility.md`. |
...
| **M10 — Skin breadth + candidate-window skinning (blocked)** | Manifest-only V1 skins; shared bounded `SkinDefinition`/`SkinCatalog`; strict user imports and revisioned IPC; built-in equal-sized `midnight`; opaque/static-tint DComp candidate restyle with normally redirected GDI first-show fallback. | M | Does not begin until M11D passes deterministic activation/visibility and the complete installed host gate. M10 owns candidate behavior/performance evidence (30 cold shows, 500 warm updates); M11 supplies only the reusable composition foundation. Plan: `docs/plans/active/m10-plan-skin-breadth-candidate-window.md`. |
| **M11 — UI aesthetic modernization + Cantonese localization (installed clone/drag proof passed; M11D pending)** | Themes/localizes the native settings panel and toolbar, ports the toolbar to DirectComposition + Direct2D, then stabilizes real-window topology with fail-closed owner recovery, asynchronous generation-safe focus handoff, same/cross-process supersession, a 250 ms watchdog, and movement-only drag with one final flush. Backdrop is build-22621-gated and falls back to an opaque static pill unless both frame and DWM backdrop setup succeed. | M–L | Installed Notepad/Chromium proof retained one valid foreground-owned HWND through repeated drag and the user reported no fresh copies. M11/M11C remain active under M11D until activation/visibility, Explorer/Electron breadth, and restart persistence pass. Plan/evidence: `docs/plans/active/m11-plan-ui-modernization-cantonese.md`, `docs/plans/active/m11c-dcomp-glass-toolbar-handoff.md`, `docs/evidence/m11/summary.md`, `docs/evidence/m11c/summary.md`. |

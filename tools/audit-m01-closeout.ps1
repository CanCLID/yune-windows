...
if ($JsonPath -eq "") {
    $JsonPath = Join-Path $RepoRoot "docs\evidence\m01\closeout\audit.json"
}
if ($MarkdownPath -eq "") {
    $MarkdownPath = Join-Path $RepoRoot "docs\evidence\m01\closeout\audit.md"
}
...
function Test-CleanupPlanEvidencePath([object]$Value) {
    return Test-ExactEvidencePath `
        -Value $Value `
        -ExpectedRelativePath "docs\evidence\m01\installer\machine-cleanup-plan.json"
}
...
function Has-LiveResultDateEvidence([string]$RelativePath) {
    ...
    $ApprovalDate = Get-IsoDateEvidenceValue "docs\evidence\m01\installer\approval.md"
...
function Get-LiveApprovalField {
    ...
    return Get-ApprovalEvidenceField "docs\evidence\m01\installer\approval.md" $Name
}
...
function Get-DiagnosticsOutputDirTranscriptPattern {
    ...
    $DiagnosticsOutputDir = Repo-Path "docs\evidence\m01\settings\registered-session-diagnostics"
...
function Get-LivePreflightPathTranscriptPattern {
    ...
    $LivePreflightPath = Repo-Path "docs\evidence\m01\installer\live-preflight.json"
...
function Get-InstallSmokeCommandPatterns {
    ...
    return @(
        "run-m01-live-smoke\.ps1(?=.*-PreflightOnly)(?=.*$LivePreflightPathTranscriptPattern)(?=.*$YuneRootTranscriptPattern)(?=.*$InstallDirTranscriptPattern)(?=.*$BrowserPathTranscriptPattern)",
...
function Get-CompletedInstallSmokeCommandPatterns {
    ...
    $LivePreflightPathTranscriptPattern = Get-LivePreflightPathTranscriptPattern
    return @(
        "^PASS\s+.*run-m01-live-smoke\.ps1(?=.*-PreflightOnly)(?=.*$LivePreflightPathTranscriptPattern)(?=.*$YuneRootTranscriptPattern)(?=.*$InstallDirTranscriptPattern)(?=.*$BrowserPathTranscriptPattern)",
...
function Get-LivePreflightCommandPattern {
    ...
    $BrowserPathTranscriptPattern = Get-BrowserPathTranscriptPattern
    $LivePreflightPathTranscriptPattern = Get-LivePreflightPathTranscriptPattern
    return "run-m01-live-smoke\.ps1(?=.*-PreflightOnly)(?=.*$LivePreflightPathTranscriptPattern)(?=.*$YuneRootTranscriptPattern)(?=.*$InstallDirTranscriptPattern)(?=.*$BrowserPathTranscriptPattern)"
}
...
function Get-CompletedLivePreflightCommandPattern {
    ...
    $BrowserPathTranscriptPattern = Get-BrowserPathTranscriptPattern
    $LivePreflightPathTranscriptPattern = Get-LivePreflightPathTranscriptPattern
    return "^PASS\s+.*run-m01-live-smoke\.ps1(?=.*-PreflightOnly)(?=.*$LivePreflightPathTranscriptPattern)(?=.*$YuneRootTranscriptPattern)(?=.*$InstallDirTranscriptPattern)(?=.*$BrowserPathTranscriptPattern)"
}
    ...
    foreach ($RelativePath in @(
        ...
        Get-ChildItem -LiteralPath $Item.FullName -Recurse -File |
            Where-Object {
                ...
                ($_.FullName -notmatch '[\\/]m01-closeout[\\/]audit\.(json|md)$')
            } |
...
function Has-PreflightReportEvidence(
    ...
    if ($RequireGeneratedAtAfterApproval) {
        $ApprovalPath = "docs\evidence\m01\installer\approval.md"
...
function Get-LivePreflightStatus {
    $LivePath = "docs\evidence\m01\installer\live-preflight.json"
    $InstallPath = "docs\evidence\m01\installer\install-preflight.json"
    $CommandsPath = "docs\evidence\m01\installer\commands.txt"
    $ApprovalPath = "docs\evidence\m01\installer\approval.md"
...
function Get-PreflightResidueCounts {
    ...
    foreach ($RelativePath in @(
            "docs\evidence\m01\installer\live-preflight.json",
            "docs\evidence\m01\installer\install-preflight.json")) {
        $Report = Read-JsonEvidence $RelativePath
        if ($null -eq $Report) {
            continue
        }
...
function Get-LivePreflightNotes([string]$Status) {
    ...
    if ((Has-Path "docs\evidence\m01\installer\approval.md") -and
        (-not (Has-CompletedLivePreflightCommandTranscript "docs\evidence\m01\installer\commands.txt"))) {
...
function Has-InstallActivationEvidence {
    $PreInstallState = "docs\evidence\m01\installer\pre-install-state.json"
    $PostInstallState = "docs\evidence\m01\installer\post-install-state.json"
    ...
    $ProfileWasActivated = (Has-ProfileSnapshotEvidence `
            -SnapshotPath "docs\evidence\m01\tsf-smoke\notepad-post-state.json" `
            -RequireInstalledFiles $true `
            -RequireRegistered $true `
            -RequireActive $true) -or
        (Has-ProfileSnapshotEvidence `
            -SnapshotPath "docs\evidence\m01\tsf-smoke\chromium-post-state.json" `
            -RequireInstalledFiles $true `
            -RequireRegistered $true `
            -RequireActive $true)
    $EarliestAppSmokeResultDate = Get-EarliestIsoDateEvidenceValue @(
        "docs\evidence\m01\tsf-smoke\notepad-smoke-result.md",
        "docs\evidence\m01\tsf-smoke\chromium-smoke-result.md"
    )
...
function Get-ExpectedM01CommitText {
...
function Get-RelativeDiagnosticsBundlePathFromInstallResult([string]$ResultPath) {
    ...
    try {
        ...
        $DiagnosticsDir = [System.IO.Path]::GetFullPath(
            (Repo-Path "docs\evidence\m01\settings\registered-session-diagnostics"))
    }
    catch {
        return $null
    }
    ...
    return Join-Path "docs\evidence\m01\settings\registered-session-diagnostics" $RelativeBundle
}
...
function Has-PassingTextSmoke([string]$ResultPath) {
    ...
    return (Has-LiveResultDateEvidence $ResultPath) -and
        ...
        ($Observed -eq (Get-ExpectedM01CommitText))
}
...
function Get-NotepadSmokeStatus {
    $result = "docs\evidence\m01\tsf-smoke\notepad-smoke-result.md"
    $commands = "docs\evidence\m01\installer\commands.txt"
    ...
    if ((Has-PassingTextSmoke $result) -and
        ...
        (Has-DistinctPngEvidence `
            "docs\evidence\m01\tsf-smoke\candidate-display-notepad.png" `
            "docs\evidence\m01\tsf-smoke\notepad-commit.png")) {
        return "complete"
    }
    return "invalid"
...
function Get-ChromiumSmokeStatus {
    $result = "docs\evidence\m01\tsf-smoke\chromium-smoke-result.md"
    $commands = "docs\evidence\m01\installer\commands.txt"
    if (-not (Has-Path $result)) {
        return "missing"
    }
    if ((Has-PassingTextSmoke $result) -and
        ...
        (Has-TextSmokePostStateForResult `
            "docs\evidence\m01\tsf-smoke\chromium-post-state.json" `
            $result) -and
        (Has-DistinctPngEvidence `
            "docs\evidence\m01\tsf-smoke\candidate-display-chromium.png" `
            "docs\evidence\m01\tsf-smoke\chromium-commit.png")) {
        return "complete"
    }
    return "invalid"
}
...
function Get-CandidateDisplayStatus([string]$NotepadStatus, [string]$ChromiumStatus) {
    $has_build_preflight = Has-Path "docs\evidence\m01\candidate-window\build-preflight.md"
...
function Get-TsfServerIpcStatus([string]$NotepadStatus) {
    $has_preflight = (Has-Path "docs\evidence\m01\tsf-smoke\server-ipc-smoke.md") -and
...
function Get-InstallActivationStatus {
    $result = "docs\evidence\m01\installer\result.md"
    $commands = "docs\evidence\m01\installer\commands.txt"
    $ApprovalNoteTranscriptPattern = Get-ApprovalNoteTranscriptPattern
    $InstallDirTranscriptPattern = Get-InstallDirTranscriptPattern
    $BrowserPathTranscriptPattern = Get-BrowserPathTranscriptPattern
    ...
    if ((Has-Path $commands) -and
        ...
        (Has-ResultDateNoEarlierThanEvidence `
            $result `
            "docs\evidence\m01\installer\cleanup-result.md") -and
...
function Get-DiagnosticsStatus {
    ...
    $InstallDirTranscriptPattern = Get-InstallDirTranscriptPattern
    $DiagnosticsOutputDirTranscriptPattern = Get-DiagnosticsOutputDirTranscriptPattern
    $DiagnosticsPreState = "docs\evidence\m01\settings\diagnostics-pre-state.json"
    $DiagnosticsDir = "docs\evidence\m01\settings\registered-session-diagnostics"
    ...
    if (Test-Path -LiteralPath $DiagnosticsPath) {
        $RelativeBundle = Get-RelativeDiagnosticsBundlePathFromInstallResult `
            "docs\evidence\m01\installer\result.md"
    ...
    if ((Has-Path "docs\evidence\m01\settings\diagnostics-export.md") -and
...
function Get-CleanupStatus {
    $result = "docs\evidence\m01\installer\cleanup-result.md"
    $commands = "docs\evidence\m01\installer\commands.txt"
    $ApprovalNoteTranscriptPattern = Get-ApprovalNoteTranscriptPattern
    $InstallDirTranscriptPattern = Get-InstallDirTranscriptPattern
    if (-not (Has-Path $result)) {
    ...
    if ((Has-Path $commands) -and
        ...
        (Has-CompletedLiveCommandTranscript $commands) -and
        (Has-CleanupEvidence `
            "docs\evidence\m01\installer\cleanup-validation.json" `
            "docs\evidence\m01\installer\post-cleanup-state.json") -and
        (Has-CleanupResultDateNoEarlierThanValidation `
            $result `
            "docs\evidence\m01\installer\cleanup-validation.json") -and
        (Has-LiveResultDateEvidence $result) -and
        (File-Contains $result "^Status:\s*passed\s*$") -and
        (-not (File-Contains $result "^Status:\s*failed\s*$")) -and
...
function Get-LiveApprovalEvidenceStatus {
    $Evidence = "docs\evidence\m01\installer\approval.md"
...
function Has-MachineCleanupSnapshotEvidence(
    ...
    $BeforeSnapshotPath = "docs\evidence\m01\installer\machine-cleanup-before.json"
    $AfterSnapshotPath = "docs\evidence\m01\installer\machine-cleanup-after.json"
...
function Has-MachineCleanupCurrentResidueCoverage(
    ...
    $CleanupPlanPath = "docs\evidence\m01\installer\machine-cleanup-plan.json"
    ...
    $BeforeSnapshot = Read-JsonEvidence "docs\evidence\m01\installer\machine-cleanup-before.json"
...
function Get-MachineCleanupApprovalEvidenceStatus {
    $Result = "docs\evidence\m01\installer\machine-cleanup-result.md"
    if (-not (Has-Path $Result)) {
        return "missing"
    }

    $Evidence = "docs\evidence\m01\installer\machine-cleanup-approval.md"
    if (-not (Has-Path $Evidence)) {
        return "invalid"
    }
    ...
    $ResultJsonPath = "docs\evidence\m01\installer\machine-cleanup-result.json"
...
function Get-ApprovalDisciplineStatus {
    $Evidence = "docs\evidence\m01\tsf-smoke\machine-state-gates.md"
    if (-not (Has-Path $Evidence)) {
        return "missing"
    }
...
function Get-ApprovalBriefAdvisoryIssue {
    $BriefPath = "docs\evidence\m01\installer\approval-brief.md"
    ...
    $PrepValidationPath = "docs\evidence\m01\installer\elevated-live-smoke-prep-validation-result.json"
    ...
    $LiveLauncherPath = "docs\evidence\m01\installer\elevated-live-smoke-launch-result.json"
...
Add-Gate `
    ...
    ($(if ((Has-Path "docs\evidence\m01\bootstrap\repo-state.md") -and
           (Has-Path "docs\evidence\m01\bootstrap\reference-audit.md") -and
           (Has-Path "docs\evidence\m01\bootstrap\process-model.md") -and
           (Has-Path "docs\evidence\m01\bootstrap\first-smoke-target.md")) { "complete" } else { "missing" })) `
    "docs/evidence/m01/bootstrap/" `
    "Task 0/1 evidence."

Add-Gate `
    "yune-host-smoke" `
    "packaged Yune host processes ngohaig with jyut6ping3" `
    ($(if ((Has-Path "docs\evidence\m01\yune-host\result.json") -and
           (File-Contains "docs\evidence\m01\yune-host\result.json" '"schema_id": "jyut6ping3"')) { "complete" } else { "missing" })) `
    "docs/evidence/m01/yune-host/result.json" `
...
Add-Gate `
    ...
    $TsfServerIpcStatus `
    "docs/evidence/m01/tsf-smoke/server-ipc-smoke.md" `
...
Add-Gate `
    ...
    $NotepadSmokeStatus `
    "docs/evidence/m01/tsf-smoke/notepad-smoke-result.md" `
...
Add-Gate `
    ...
    "docs/evidence/m01/candidate-window/" `
...
Add-Gate `
    ...
    $ChromiumSmokeStatus `
    "docs/evidence/m01/tsf-smoke/chromium-smoke-result.md" `
...
Add-Gate `
    ...
    $DiagnosticsStatus `
    "docs/evidence/m01/settings/diagnostics-export.md" `
...
Add-Gate `
    ...
    $CleanupStatus `
    "docs/evidence/m01/installer/cleanup-result.md" `
...
Add-Gate `
    ...
    ($(if ((Has-Path "docs\evidence\m01\settings\webview2-spike.md") -and
           (File-Contains "docs\evidence\m01\settings\webview2-spike.md" 'Decision: `defer-settings`')) { "complete" } else { "missing" })) `
    "docs/evidence/m01/settings/webview2-spike.md" `
    "Decision is defer-settings for M01."

Add-Gate `
    "approval-discipline" `
    "machine-state scripts require explicit approval" `
    $ApprovalDisciplineStatus `
    "docs/evidence/m01/tsf-smoke/machine-state-gates.md; docs/evidence/m01/installer/approval.md; docs/evidence/m01/installer/machine-cleanup-approval.md" `
...
if ($OverallStatus -eq "complete") {
    $Lines += "M01 closeout gates are complete."
} else {
    $Lines += "M01 must remain open until every gate is complete."
}
...
if ($RequireComplete -and $OverallStatus -ne "complete") {
    ...
    throw "M01 closeout audit incomplete: $($IncompleteGates -join ', ')"
}

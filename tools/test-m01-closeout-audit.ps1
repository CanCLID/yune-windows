...
if ($OutputDir -eq "") {
    $OutputDir = Join-Path $env:TEMP "yune-windows\m01-closeout-audit-test"
}
...
$Audit = Join-Path $RepoRoot "tools\audit-m01-closeout.ps1"
if (-not (Test-Path -LiteralPath $Audit)) {
    throw "missing M01 closeout audit script: $Audit"
}
...
if ($Result.status -ne $ExpectedOverallStatus) {
    throw "M01 closeout audit status should be $ExpectedOverallStatus when non-complete gate count is $($IncompleteIds.Count), got $($Result.status)"
}
if ($Result.status -eq "complete") {
    foreach ($StaleCompleteNote in @(
            ...
            'M01 must remain open until every gate is complete')) {
    ...
    if ($Markdown -notmatch [regex]::Escape("M01 closeout gates are complete.")) {
...
if (-not $AnyPreflightEvidence) {
    ...
    if (-not (Test-Path -LiteralPath $SummaryPath)) {
        throw "missing raw live preflight evidence and compact M01 summary"
    }
    $Summary = Get-Content -Raw -LiteralPath $SummaryPath | ConvertFrom-Json
    if (($Summary.status -ne "complete") -or
        ($Summary.evidence_policy -ne "compact-summary-only")) {
        throw "M01 summary must document complete compact-summary evidence when raw live preflight artifacts are pruned"
    }
...
$CommandsPath = Join-Path $RepoRoot "docs\evidence\m01\installer\commands.txt"
...
Write-Host "M01 closeout audit smoke passed: status=$($Result.status), incomplete=$($IncompleteIds.Count)"

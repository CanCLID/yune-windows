...
M02 through M09 are complete. **M11 is the active stop-the-line track:** its
ownership/drag/backdrop repair is implemented non-elevated, but installed visual
proof remains open. **M10 is blocked** until that gate passes and continues to own
skin breadth plus candidate-window rendering.
...
| **M10 — Skin breadth + candidate-window skinning (blocked)** | Manifest-only V1 skins; shared bounded `SkinDefinition`/`SkinCatalog`; strict user imports and revisioned IPC; built-in equal-sized `midnight`; opaque/static-tint DComp candidate restyle with normally redirected GDI first-show fallback. | M | Does not begin until the installed M11 gate proves one foreground-owned, clone-free toolbar. M10 owns candidate behavior/performance evidence (30 cold shows, 500 warm updates); M11 supplies only the reusable composition foundation. Plan: `docs/plans/active/m10-plan-skin-breadth-candidate-window.md`. |
| **M11 — UI aesthetic modernization + Cantonese localization (implementation fixed; installed proof pending)** | Themes/localizes the native settings panel and toolbar, ports the toolbar to DirectComposition + Direct2D, then stabilizes real-window topology with fail-closed owner recovery, asynchronous generation-safe focus handoff, same/cross-process supersession, a 250 ms watchdog, and movement-only drag with one final flush. Backdrop is build-22621-gated and falls back to an opaque static pill unless both frame and DWM backdrop setup succeed. | M–L | Approval-gated installed Notepad/Chromium/Explorer/Electron proof must show one valid foreground-owned visible HWND, stable HWND identity through drag, previous-host hide within 250 ms, no copies/afterimages, no focus steal, and persisted position. Failure follows acrylic → static DComp → normally redirected opaque native D2D. Plan/evidence: `docs/plans/active/m11-plan-ui-modernization-cantonese.md`, `docs/plans/active/m11c-dcomp-glass-toolbar-handoff.md`, `docs/evidence/m11/summary.md`, `docs/evidence/m11c/summary.md`. |
